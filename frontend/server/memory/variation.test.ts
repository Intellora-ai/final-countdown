...
import {
  MIN_WORDS_A_NEW_EXPLANATION_ADDS,
  noveltyAgainst,
  type Novelty,
} from './variation.ts'
...
/* ==========================================================================
 * THE RULE ITSELF MOVED BACK TO `server/memory/variation.ts`.
 *
 * It lived here because the reachability gate reported it as an orphan --
 * "built and tested, imported by nothing that ships" -- and the note left in
 * its place said it "moves back out the moment something shipping calls it".
 *
 * `handler.ts`'s `conceptFor` is that caller: it compares a newly written
 * explanation against the wording of every prior one before serving it. The
 * proofs below are unchanged and now import what they prove.
 * ========================================================================== */
