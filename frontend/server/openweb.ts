/**
 * THE OPEN-WEB SEARCH CORE, SHARED BY THE DEV SERVER AND THE PRODUCTION ONE.
 *
 * This code was born inside `vite-plugin-search.ts` and moved here VERBATIM
 * when the production server wired the same route: importing the plugin file
 * from `server/` dragged vite's middleware types into the server project and
 * lit up errors in code no change had touched -- the exact trap
 * `canvasContract.test.ts` documents for the canvas boundary. The plugin now
 * imports from HERE and stays a thin vite adapter, so dev and prod answer
 * /api/search through one function and cannot drift.
 *
 * WHY `server/` AND NOT `src/websearch/`. The two env variable NAMES below are
 * the credential's address, and `island.test.ts` refuses any occurrence of
 * them under `src/` -- everything there is compiled into something a browser
...
import type { SearchHit } from '../src/websearch/port.ts'
...
export interface SearchedPage {
  ...
  /**
   * WHICH shapes of instruction were found -- `injectionSignals` kinds, from
   * the guard run the pipeline already did. `suspicious` collapses these to a
   * boolean for the one decision a caller usually makes; the kinds are kept
   * because M7's law is "flag every shape and censor none", and a boolean
