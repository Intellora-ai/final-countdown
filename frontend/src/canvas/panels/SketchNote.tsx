...
import { color, ink, overlay, space, spaceLegacy, type, typeLegacy } from '../design/tokens'
...
export function SketchNote({ width = 300 }: { width?: number }) {
  return (
    <div style={{ width, display: 'flex', flexDirection: 'column', gap: space.xs }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: spaceLegacy.hair }}>
        <span style={{
          fontFamily: "'Fraunces', Georgia, serif", fontStyle: 'italic',
          fontSize: typeLegacy.annotation.size, color: ink.annotation, borderBottom: `1px solid ${overlay.rule}`,
          paddingBottom: spaceLegacy.tight,
        }}>
      ...
      <svg width={width} height={128} aria-hidden="true">
        ...
        <g transform="translate(6,10)">
          <line x1={10} y1={96} x2={10} y2={8} stroke={overlay.strong} strokeWidth={1.1} />
          <line x1={10} y1={96} x2={104} y2={96} stroke={overlay.strong} strokeWidth={1.1} />
          ...
          {[[30, 74], [48, 58], [66, 42]].map(([x, y], i) => (
            <line key={i} x1={x - 8} y1={y + 8} x2={x} y2={y} stroke={overlay.mark} strokeWidth={1} />
          ))}
          <path d="M24 40 L38 30" stroke={overlay.mark} strokeWidth={1} />
          <path d="M34 28 L40 29 L38 34" stroke={overlay.mark} strokeWidth={1} fill="none" />
        </g>

        {/* right: the clean P vs T line with the 300 K drop-line */}
        <g transform="translate(126,10)">
          <line x1={14} y1={96} x2={14} y2={6} stroke={overlay.strongest} strokeWidth={1.1} />
          <line x1={14} y1={96} x2={150} y2={96} stroke={overlay.strongest} strokeWidth={1.1} />
          <path d="M10 4 L14 -2 L18 4" stroke={overlay.strongest} strokeWidth={1.1} fill="none" />
          <path d="M148 92 L154 96 L148 100" stroke={overlay.strongest} strokeWidth={1.1} fill="none" />
          <text x={2} y={2} fontSize={12} fill={ink.annotation} fontFamily="'Fraunces', Georgia, serif" fontStyle="italic">P</text>
          <text x={152} y={110} fontSize={12} fill={ink.annotation} fontFamily="'Fraunces', Georgia, serif" fontStyle="italic">T</text>
          <path d="M18 90 L140 20" stroke={color.accent} strokeWidth={1.6} strokeLinecap="round" />
          <circle cx={80} cy={55} r={3} fill={color.accent} />
          <line x1={80} y1={58} x2={80} y2={96} stroke={overlay.guide} strokeDasharray="3 3" />
          <text x={68} y={110} fontSize={9.5} fill={color.textMuted} fontFamily="ui-monospace, monospace">300K</text>
          <text x={96} y={30} fontSize={10.5} fill={ink.annotation} fontFamily="'Fraunces', Georgia, serif" fontStyle="italic">(P=kT)</text>
        </g>
      </svg>

      <div style={{ fontSize: typeLegacy.sketchLaw.size, color: ink.equation, marginTop: -6 }}>
        <Katex latex="PV = nRT" />
      </div>
    </div>
  )
