...
describe('LAW S9-M2b -- the model is asked, inside the one call, and its answer comes back', () => {
  it('the writer is told the closed list and the hint; the reply carries asked and shown; one call', async () => {
    ...
    expect(server.lastSystem(), 'the rules hint (graph) was not offered to the writer').toMatch(/read her words as: graph/)
    expect(server.lastSystem(), 'her own sentence never reached the writer').toContain('graph how pressure changes with temperature')
