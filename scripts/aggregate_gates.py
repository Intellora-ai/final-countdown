...
def job_conclusions(path: Path | None) -> dict[str, str]:
    ...
    try:
        ...
        print(
            f"  job conclusions unavailable ({exc}); "
...
def main() -> int:
    ...
    for name, g in gates.items():
        ...
        g["job_conclusion"] = conclusion
        g["note"] = (
            f"the report says {g['status']} but GitHub concluded "
        ...
        g["status"] = (
            ...
            else ("SKIPPED" if conclusion == "skipped" else "INFRASTRUCTURE_FAILURE")
        )
        if conclusion == "cancelled":
    ...
    for name, g in gates.items():
        ...
        if conclusion == "cancelled" and g.get("evidence") is None:
            g["status"] = "CANCELLED"
            g["note"] = (
                "the job was cancelled, so no report was written — "
            ...
            g["status"] = "SKIPPED"
            g["note"] = (
                "the job was skipped. GitHub counts a skipped job as "
    ...
    print(
        f"  cancelled_checks={len(cancelled_checks)}  "
