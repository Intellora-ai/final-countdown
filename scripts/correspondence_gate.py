...
def sources() -> dict[str, Path]:
    ...
    return {p.stem: p for p in sorted(SRC.glob("*.py")) if not p.name.startswith("_")}


def committed(name: str) -> tuple[Path, Path]:
    return (
        SPECS / f"{name}_semantics_spec.lean",
...
def verify_with_axle(spec: Path, proof: Path) -> tuple[bool, str]:
    ...
    if payload.get("okay") is True:
        ...
        if not axioms:
            return False, (
                "no axiom report — the proof file must end with "
                "`#print axioms`, or the audit is not running"
            )
        extra = axioms - FOUNDATIONAL
        if extra:
            return False, (
                f"proof depends on non-foundational axioms: {', '.join(sorted(extra))}"
            )
...
def main() -> None:
    with Gate(
        "correspondence", version="1.0.0", tools={"axle": ["axle", "--version"]}
    ) as g:
        src_map = sources()
        spec_names = {
        ...
        g.check(
            "every source has a correspondence pair",
            not missing,
            ", ".join(missing) or "all covered",
        )
        for name in missing:
            g.fail(
                what=f"src/{name}.py has no correspondence pair",
        ...
        g.check(
            "every correspondence pair has a source",
            not orphans,
            ", ".join(orphans) or "all matched",
        )
        for name in orphans:
            g.fail(
                what=f"'{name}' has a correspondence pair but no source",
        ...
        if not src_map:
            g.fail(
                what="there are no sources to correspond to",
        ...
        with tempfile.TemporaryDirectory() as tmp:
            for name, path in src_map.items():
                try:
                    ...
                    g.fail(
                        what=f"src/{name}.py is outside the supported subset",
                        where=str(path),
                        why=str(exc),
                ...
                fresh = on_disk_spec == fresh_spec and on_disk_proof == fresh_proof
                g.check(
                    f"{name}: pair matches current source",
                    fresh,
                ...
                if not fresh:
                    ...
                    g.fail(
                        what=f"{name}: the committed proof describes different "
                        "Python than src/",
                        where=str(spec_path),
                ...
                g.check(
                    f"{name}: proof states the quantified denotation",
                    has_denotation,
                    denotes
                    if has_denotation
                ...
                if not has_denotation:
                    ...
                    g.fail(
                        what=f"{name}: the proof carries no universally "
                        ...
                        + ("" if stated else f"; `theorem {denotes}` is absent")
                        + (
                            ""
                            if audited
                            else f"; `#print axioms {denotes}` is absent, so "
                ...
                g.check(
                    f"{name}: kernel accepts denotation + correspondence + property",
                    ok,
                    detail or "okay",
                )
                if ok:
                    print(
                        f"✓ {name}: denotation, correspondence and property "
                        "kernel-checked"
                    )
                    ...
                    g.fail(
                        what=f"{name}: the kernel rejected the file",
                        where=f"{spec_path} + {proof_path}",
                        why=detail,
        ...
        g.set_scope(
            stale=len(stale), rejected=len(rejected), unavailable=len(unavailable)
        )
        g.artifact("reports/correspondence.json")
        if stale or rejected:
            ...
            print(
                f"✓ {len(src_map)} function(s): the theorems are about the "
                "Python in this tree"
            )
            g.passed()
