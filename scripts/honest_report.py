...
def report(spec_files: list[str], threshold: float = 0.9) -> str:
    ...
    proof = (
        "VERIFIED" if all(p["proof"] == "VERIFIED" for p in proofs) else "UNVERIFIED"
    )
    ...
    print(
        f"  Indistinguishable on sample: {mut['equivalent_excluded']} excluded "
        f"(not proven equivalent)"
    )
    print(f"  Boundary coverage:        {boundary}/{len(spec_files)} specs probed")
    print(
        f"  Joint strength:           {comp['joint_strength']:.2f}"
        f"   individual {[f'{v:.2f}' for v in comp['individual_strengths']]}"
    )
    sc = suff["scope"]
    scoped = (
        f"{suff['sufficiency']} (within scope: {sc['domain']})"
        if suff["sufficiency"] == "ESTABLISHED"
        else suff["sufficiency"]
    )
    ...
    print(
        f"  Search depth:             {sc['examples_per_property']} examples per property"
    )
    ...
    if overall == "PASS_WITH_LIMITATIONS":
        ...
        for d in suff.get("derivations", []):
            i = d["diverges_at"]
            print(
                f"    witness {d['witness']}: at {i['inputs']} "
                f"original={i['original']} witness={i['witness']}"
            )
            if d["anchor_refutes"]:
                print(f"      anchor refutes it: {d['anchor_refutes']}")
                print(
                    f"      derivable spec (from the anchor): {d['derived_from_anchor']}"
                )
            else:
                print(
                    "      anchor is SILENT — state the intent, do not invent a property"
                )
