...
/-!
DENOTATION, CORRESPONDENCE AND PROPERTY FOR `src/multiply.py`.

  function      multiply(a, b)
  source sha256 6d008a4b7b62e93b5da08a9da78966fdeb034c15cf8bf4b526bd1a977e97c270
  guards        0

`multiply_ast` below is the syntax tree of that exact source, emitted as data by
scripts/pysem.py. Every theorem here is about `evalFunc multiply_ast` — the
...
THE CHAIN NEEDS BOTH KERNEL LINKS, AND THEY DO DIFFERENT JOBS.
  ...
  DENOTATION      `evalFunc multiply_ast args = <closed form>` for ALL integers.
  ...
  CORRESPONDENCE  `evalFunc multiply_ast [pt] = <observed>` at 12 points, by
...
from `multiply_ast` itself, so it is not a second hand-written model that could
drift — and it is not trusted either, because the kernel is what closes the
gap between it and `eval`. -/
theorem multiply_ast_denotes (a b : Int) :
    evalFunc multiply_ast [a, b] = some (a * b) := by
  simp [evalFunc, multiply_ast, runGuards, eval, lookupVar]
...
contact with Python. -/
theorem multiply_ast_matches_cpython :
    evalFunc multiply_ast [(-7 : Int), (0 : Int)] = some (0 : Int)
    ∧
    evalFunc multiply_ast [(-7 : Int), (-7 : Int)] = some (49 : Int)
    ∧
    evalFunc multiply_ast [(-3 : Int), (1 : Int)] = some (-3 : Int)
    ∧
    evalFunc multiply_ast [(-3 : Int), (-3 : Int)] = some (9 : Int)
    ∧
    evalFunc multiply_ast [(-1 : Int), (2 : Int)] = some (-2 : Int)
    ∧
    evalFunc multiply_ast [(-1 : Int), (-1 : Int)] = some (1 : Int)
    ∧
    evalFunc multiply_ast [(0 : Int), (3 : Int)] = some (0 : Int)
    ∧
    evalFunc multiply_ast [(0 : Int), (0 : Int)] = some (0 : Int)
    ∧
    evalFunc multiply_ast [(1 : Int), (5 : Int)] = some (5 : Int)
    ∧
    evalFunc multiply_ast [(1 : Int), (1 : Int)] = some (1 : Int)
    ∧
    evalFunc multiply_ast [(2 : Int), (8 : Int)] = some (16 : Int)
    ∧
    evalFunc multiply_ast [(2 : Int), (2 : Int)] = some (4 : Int) := by
...
theorem multiply_ast_is_multiplication (a b : Int) :
    evalFunc multiply_ast [a, b] = evalFunc multiply_ast [b, a]
    ∧ evalFunc multiply_ast [a, (1 : Int)] = some a := by
  refine ⟨?_, ?_⟩
  · simp [evalFunc, multiply_ast, runGuards, eval, lookupVar, Int.mul_comm]
  · simp [evalFunc, multiply_ast, runGuards, eval, lookupVar]

#print axioms multiply_ast_denotes
#print axioms multiply_ast_matches_cpython
#print axioms multiply_ast_is_multiplication
