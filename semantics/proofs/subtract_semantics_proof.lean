...
/-!
DENOTATION, CORRESPONDENCE AND PROPERTY FOR `src/subtract.py`.
...
THE CHAIN NEEDS BOTH KERNEL LINKS, AND THEY DO DIFFERENT JOBS.
  ...
  DENOTATION      `evalFunc subtract_ast args = <closed form>` for ALL integers.
  ...
  CORRESPONDENCE  `evalFunc subtract_ast [pt] = <observed>` at 12 points, by
...
from `subtract_ast` itself, so it is not a second hand-written model that could
drift — and it is not trusted either, because the kernel is what closes the
gap between it and `eval`. -/
theorem subtract_ast_denotes (a b : Int) :
    evalFunc subtract_ast [a, b] = some (a - b) := by
...
#print axioms subtract_ast_denotes
