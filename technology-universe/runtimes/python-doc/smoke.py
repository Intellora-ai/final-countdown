...
def run(id,dist,fn):
    ...
    except Exception as e: R.append((False,id,dist,'?',f'version lookup failed: {e}')); return
    try: R.append((True,id,dist,v,fn()))
    except Exception as e: R.append((False,id,dist,v,traceback.format_exc().strip().split('\n')[-1]))

def t_rdflib():
    import rdflib
    g=rdflib.Graph(); g.parse(data='<http://a/s> <http://a/p> "lit" .',format='nt')
...
def t_pyshacl():
    ...
    shapes=('@prefix sh: <http://www.w3.org/ns/shacl#> . @prefix ex: <http://ex/> .'
            'ex:S a sh:NodeShape ; sh:targetClass ex:Person ; '
            'sh:property [ sh:path ex:name ; sh:minCount 1 ] .')
    conforms,_,txt=validate(data,shacl_graph=shapes,data_graph_format='turtle',shacl_graph_format='turtle')
...
def t_pypdf():
    ...
    r=PdfReader('fixture.pdf'); txt=r.pages[0].extract_text()
    assert 'REGISTRY' in txt, repr(txt)
    w=PdfWriter(); w.add_page(r.pages[0]); w.write('pypdf-out.pdf')
    return f'read {len(r.pages)}pg, extracted {txt.strip()!r}, rewrote to pypdf-out.pdf'

def t_pymupdf():
    import pymupdf
    d=pymupdf.open('fixture.pdf'); txt=d[0].get_text().strip()
    assert 'REGISTRY' in txt, repr(txt)
    pix=d[0].get_pixmap(dpi=100); pix.save('pymupdf-out.png')
    return f'{d.page_count}pg, text={txt!r}, rasterised {pix.width}x{pix.height} png'

def t_pillow():
    from PIL import Image
    im=Image.new('RGB',(90,40),(10,120,200)); im.save('pillow-out.png')
...
def t_cv2():
    ...
    sm=cv2.resize(img,(45,20)); gr=cv2.cvtColor(sm,cv2.COLOR_BGR2GRAY)
...
for ok,id,d,v,msg in R: print(f"{'PASS' if ok else 'FAIL'} id={id} {d}@{v} :: {msg}")
