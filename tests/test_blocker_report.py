...
def manifest(
    gates: dict[str, Any], blocking: list[str], overall: str = "FAIL"
) -> dict[str, Any]:
    return {
        "overall": overall,
...
def one_failure(**over: str) -> dict[str, str]:
    base = {
        "what": "a thing went wrong",
        "where": "scripts/gate.py:1",
...
def test_every_recorded_field_reaches_the_log() -> None:
    ...
    text, _ = blocker_report.render(
        manifest({"bandit": {"status": "FAIL", "failures": [f]}}, ["bandit"]), MERGEABLE
    )
...
def test_a_gate_with_no_report_is_still_a_blocker() -> None:
    ...
    """
    gates: dict[str, Any] = {
        "mutmut": {"status": "UNKNOWN", "note": "no report produced", "failures": []}
    }
...
def test_the_counts_separate_ran_and_failed_from_never_concluded() -> None:
    ...
    gates: dict[str, Any] = {
        "a": {"status": "PASS"},
        "b": {"status": "FAIL", "failures": []},
        "c": {"status": "UNKNOWN"},
        "d": {"status": "INFRASTRUCTURE_FAILURE"},
    }
...
def test_a_real_file_with_an_unprovable_line_is_annotated_without_one(
    ...
    assert "line=" not in text, f"claimed a line it could not prove: {where!r}"
...
def test_the_same_defect_keeps_its_id_across_runs() -> None:
    f = one_failure()
    assert blocker_report.failure_id("bandit", f) == blocker_report.failure_id(
        "bandit", dict(f)
    )
...
def test_a_crashing_renderer_does_not_turn_a_failure_into_a_pass(
    ...
    (work / "ci" / "gates.toml").write_text(
        (REPO / "ci" / "gates.toml").read_text(encoding="utf-8"), encoding="utf-8"
    )
    ...
    r = subprocess.run(
        [sys.executable, "scripts/aggregate_gates.py", "--evidence-root", "evidence"],
        cwd=work,
        capture_output=True,
        text=True,
        timeout=120,
...
def test_a_warning_is_visible_on_a_completely_green_run() -> None:
    ...
    text, md = blocker_report.render(manifest(gates, [], overall="PASS"), MERGEABLE)
...
def test_warnings_are_counted_but_do_not_change_the_verdict() -> None:
    ...
    text, _ = blocker_report.render(manifest(gates, [], overall="PASS"), MERGEABLE)
...
def test_a_warning_with_a_real_location_becomes_a_warning_annotation() -> None:
    ...
    gates: dict[str, Any] = {
        "lint": {"status": "PASS", "warnings": ["scripts/gate.py:1 unused import"]}
    }
    text, _ = blocker_report.render(manifest(gates, [], overall="PASS"), MERGEABLE)
...
def test_no_gate_job_depends_on_another_gate_job() -> None:
    ...
    assert gate_jobs <= full_needs, (
        f"the finalizer does not wait for every gate: {sorted(gate_jobs - full_needs)}"
    )
...
def test_the_independence_note_appears_only_for_multiple_blockers() -> None:
    ...
    two: dict[str, Any] = {
        "a": {"status": "FAIL", "failures": [one_failure()]},
