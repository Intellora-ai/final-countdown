...
def test_enforce_spec_reports_every_failing_check_not_the_first(tmp_path: Path) -> None:
...
def test_the_security_gate_reports_every_unresolved_finding(tmp_path: Path) -> None:
...
def test_pyright_errors_become_one_finding_each_with_the_rule_name(
    ...
    text = PYRIGHT_OUTPUT.replace(
        "/home/runner/work", str(tmp_path / "home" / "runner" / "work")
    )
...
def test_a_diagnostic_without_a_rule_name_does_not_borrow_the_next_one(
    ...
    text = (
        "  scripts/gate.py:1:1 - error: a syntax error has no rule name\n"
...
def test_output_with_no_recognised_defect_yields_no_findings() -> None:
    ...
    """
    assert (
        run_gate.extract_findings("Traceback (most recent call last):\n  MemoryError\n")
        == []
    )
...
def test_a_passing_run_produces_no_findings() -> None:
    ...
    text = (
        "✓ specs/add_spec.lean: no precondition to be vacuous about\n"
...
def test_the_wrapper_writes_one_report_entry_per_defect(tmp_path: Path) -> None:
    ...
    report = json.loads(
        (work / "reports" / "vacuity-check.json").read_text(encoding="utf-8")
    )
...
def test_a_failure_no_extractor_recognises_is_still_recorded(tmp_path: Path) -> None:
    ...
    report = json.loads((work / "reports" / "opaque.json").read_text(encoding="utf-8"))
    assert report["status"] == "FAIL"
