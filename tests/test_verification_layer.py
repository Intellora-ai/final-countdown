...
def run_gate(
    cwd: Path, name: str, command: Sequence[str], *, timeout: int | None = None
) -> subprocess.CompletedProcess[str]:
...
def test_wrapped_command_that_times_out_is_infrastructure_failure_not_fail(
    ...
    """
    r = run_gate(tmp_path, "slow", [PY, "-c", "import time; time.sleep(30)"], timeout=1)
...
def test_garbage_on_stdout_with_a_zero_exit_is_judged_by_the_exit_code(
    ...
    assert r.returncode == 0, (
        f"a successful command was failed by its own noise\n{r.stdout}"
    )
...
def test_security_gate_does_not_call_an_unscannable_file_clean(tmp_path: Path) -> None:
    ...
    """
    (tmp_path / "src").mkdir()
    (tmp_path / "src" / "fine.py").write_text(
        "def f() -> int:\n    return 1\n", encoding="utf-8"
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "line",
            "-- def foo is the subject of this contract\n-- nothing is defined here\n",
        ),
        ("block", "/-\ndef foo (x : Int) : Int := x\n-/\n"),
    ],
)
def test_spec_source_comment_only_spec_is_nonzero(
    tmp_path: Path, style: str, body: str
) -> None:
...
def test_gate_whose_body_raises_is_infrastructure_failure_with_the_traceback(
    ...
    r = gate_driver(
        tmp_path, "boom", "    raise RuntimeError('the verifier itself exploded')"
    )
...
def test_a_stripped_path_never_produces_a_green_gate(
    tmp_path: Path, script: str, args: list[str], expect: str
) -> None:
